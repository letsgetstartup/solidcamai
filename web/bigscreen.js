...
(function () {
    ...
    const tenant = qs.get("tenant") || "tenant_demo";
    const site = qs.get("site") || "site_demo";
    const dev = qs.get("dev") === "1";
    const displayToken = qs.get("display_token") || "";

    // Match dashboard.js pattern: API_BASE is the root, route includes /portal_api/v1
    const API_BASE = (dev)
        ? "http://127.0.0.1:8081"
        : "https://portal-api-i6yvrrps6q-uc.a.run.app";
